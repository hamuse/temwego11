...
import com.wego.web.contents.CommunityMapper;
import lombok.Data;
@Data
@Lazy
@Component
public class CommProxy extends Proxy{
    ...
    public String comment() {
        List<String> fcomments = Arrays.asList("첫 방문인데", "세번째 방문인데", "여러번 방문인데", "다시 오고싶은", "훌륭한", "다시한번 오고싶은",
                "두번째 방문인데", "자주왔지만 다음엔");
        ...
        return fcomments.get(0) + comments.get(0);
    }
    public String content() {
    	trunk.put(Arrays.asList("site","srch"), Arrays.asList("직접입력","스톤애견풀빌라"));
        ArrayList<String> t = crawlingProxy.choose(trunk.get()).get();
    ...
    public Community makeCommu() {
        ...
        public void insertCommunity() {
            for(int i = 1; i <=5; i++) {
                communityMapper.insertComm(makeCommu());
            }
        }
